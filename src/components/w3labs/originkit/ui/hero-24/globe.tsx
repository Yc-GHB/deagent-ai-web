...
const DEFAULT_DOTS_CONFIG: DotsConfig = {
    color: "#ffffff",
    size: 5,
    density: 8,
    allDots: false,
...
export default function Globe({
    ...
    const markers = markerConfig.markers;
    ...
    useEffect(() => {
        ...
        try {
            ...
            setError("WebGL could not be initialized.");
            return () => {
                disposed = true;
                abortController.abort();
            };
        ...
        const loadWorldData = async () => {
            try {
                const response = await fetch(
                    "/originkit/hero-24/ne_50m_land.json",
                    { signal: abortController.signal }
                );
                if (!response.ok) throw new Error("Failed to load land data");
                const landFeatures = (await response.json()) as LandFeatureCollection;
                if (disposed) return;
        ...
        return () => {
            ...
            if (globeOutlineMesh) {
                ...
                const materials = Array.isArray(globeOutlineMesh.material) ? globeOutlineMesh.material : [globeOutlineMesh.material];
